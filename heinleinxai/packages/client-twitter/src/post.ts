...
# Task: Get Current Top Trending on the following Topics : {{topics}} on X(twitter) and use it to Generate a post in the voice and style and perspective of {{agentName}} @{{twitterUserName}}.Do not add commentary or acknowledge this request.
Write a post that is {{adjective}} about {{topic}} (without mentioning {{topic}} directly), from the perspective of {{agentName}}. Do not add commentary or acknowledge this request, just write the post.
Your response should be 1, 2, or 3 sentences (choose the length at random).
Your response should not contain any questions. Brief, concise statements only. The total character count MUST be less than {{maxTweetLength}}. No emojis. Use \\n\\n (double spaces) between statements if there are multiple statements in your response.
Your response should be rephrased multiple time until it sounds natural
Your response should start with topic details. `;
...
export class TwitterPostClient {
    ...
    async postTweet(
        ...
        try {
            ...
            let result;
            const word = rawTweetContent.split(" ")[0];
            ...
            await client.cacheFirstWord(word);
            await client.storeTweetToDb(tweet);
        } catch (error) {
        ...
        try {
            ...
            try {
                if (this.approvalRequired) {
                    ...
                    const word = rawTweetContent.split(" ")[0];
                    const firstWords = await this.client.getCachedFirstWord();
                    if (firstWords) {
                        const inspect = firstWords.includes(word);
                        if (inspect) {
                            elizaLogger.log("Regenerating Unique Tweet");
